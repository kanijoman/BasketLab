...
from .lineup_analysis_window import LineupAnalysisWindow
...
class BasketballSeasonApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Lineup Analysis Button
        self.lineup_analysis_button = QPushButton("👥 Análisis de Quintetos")
        self.lineup_analysis_button.clicked.connect(self.on_view_lineup_analysis)
        self.lineup_analysis_button.setEnabled(False)  # Disabled by default
        self.lineup_analysis_button.setStyleSheet("""
            QPushButton {
        ...
        """)
        layout.addWidget(self.lineup_analysis_button)
    ...
    def _validate_selections(self) -> None:
        ...
        self.lineup_analysis_button.setEnabled(all_selected)
    ...
    def on_view_lineup_analysis(self) -> None:
        """Handle lineup analysis button click."""
        try:
            if not self.db_handler.is_connected():
                QMessageBox.critical(self, "Error", "No hay conexión con MongoDB. Por favor, verifique el servidor.")
                return
            ...
            if self.current_scope == "FEB":
                ...
                if not all([competition, season_text, group_text]):
                    QMessageBox.warning(self, "Aviso",
                                      "Por favor, seleccione todas las opciones antes de ver el análisis de quintetos.")
                    return
                collection_name = self.db_handler.get_collection_name(competition, season_text, group_text)
            elif self.current_scope == "FBCYL":
                gender = self.gender_combo.currentText()
            ...
            # Create and show lineup analysis window
            self.lineup_window = LineupAnalysisWindow(
                db_handler=self.db_handler,
                collection_name=collection_name,
                scope=self.current_scope,
                season=season_text,
            ...
            self.progress_bar.setVisible(False)
            self.progress_label.setVisible(False)
            QMessageBox.critical(self, "Error", f"Error al abrir análisis de quintetos: {str(e)}")
