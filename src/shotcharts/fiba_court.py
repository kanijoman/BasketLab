...
class FIBACourt:
    ...
    @property
    def three_point_corner_to_break(self) -> float:
        """Calculate the Y coordinate where the three-point corner lines meet the arc (break point)."""
        dx = self.three_point_side_offset - self.court_center_x
        return self.hoop_center_y + np.sqrt(self.three_point_radius**2 - dx**2)
